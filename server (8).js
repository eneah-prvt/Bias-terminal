...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    // Use ag=on to get ES futures prices directly (no manual conversion needed)
    const agParam = symbol === 'SPY' ? '&ag=on' : '';

    const [wallsRes, snapRes] = await Promise.all([
      fetch(`https://www.free-flow.site/public/walls?symbol=${symbol}&exp=${exp}${agParam}`, { headers: { 'X-API-Key': FF_KEY } }),
    ...
    console.log(`GEX ${symbol} walls raw:`, JSON.stringify(walls).substring(0, 200));
    ...
    const result = {
      ...
      spy_spot:         snap.spot ?? walls.spot ?? null,
      net_gex_label:    null,
      expiration:       exp,
      // Flag that SPY levels are already in ES prices when ag=on
      is_es_prices:     symbol === 'SPY'
    };
...
// Identity function for SPY (ag=on returns ES prices directly)
function identity(price) { return price ? +parseFloat(price).toFixed(2) : null; }

// QQQ to NQ — dynamic multiplier from live prices
...
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  ...
  // SPY uses ag=on so levels are already in ES futures prices — no conversion needed
  const spyGex = formatGex(gexCache.SPY, 'ES (via SPY)', identity);
