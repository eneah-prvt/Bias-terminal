...
// Daily ratio cache — calculated during market hours, reused after close
const dailyRatio = { SPY: 10.10, QQQ: 41.267, updatedAt: null };
...
function updateDailyRatio(symbol, ffSpot) {
  ...
  const now = new Date();
  const utcH = now.getUTCHours();
  const isMarketHours = utcH >= 13 && utcH < 21;
  if (!isMarketHours) return; // Only update during NY session
  const newRatio = futuresPrice / ffSpot;
  if (newRatio > 5 && newRatio < 50) { // Sanity check
    dailyRatio[symbol] = newRatio;
    dailyRatio.updatedAt = now.toISOString();
    console.log(\`Daily ratio \${symbol} updated: \${newRatio.toFixed(4)}\`);
  }
}
...
app.get('/api/gex/raw', async (req, res) => {
  ...
  try {
    ...
    const today = new Date().toISOString().split('T')[0];
    const exps = expD.expirations || [];
    const exp = exps.filter(e => e > today)[0] || exps[0];

    // Test with ag=on
    const [wallsNoAg, wallsAg] = await Promise.all([
      fetch(`https://www.free-flow.site/public/walls?symbol=SPY&exp=${exp}`, { headers: { 'X-API-Key': FF_KEY } }),
      fetch(`https://www.free-flow.site/public/walls?symbol=SPY&exp=${exp}&ag=on`, { headers: { 'X-API-Key': FF_KEY } })
    ]);
    const noAg = wallsNoAg.ok ? await wallsNoAg.json() : { error: wallsNoAg.status };
    const withAg = wallsAg.ok ? await wallsAg.json() : { error: wallsAg.status };

    res.json({
      exp,
      without_ag: { spot: noAg.spot, call: noAg.call_wall?.strike, put: noAg.put_wall?.strike, flip: noAg.gamma_flip },
      with_ag:    { spot: withAg.spot, call: withAg.call_wall?.strike, put: withAg.put_wall?.strike, flip: withAg.gamma_flip },
    });
