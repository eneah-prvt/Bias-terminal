...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    // Calculate conversion ratio at fetch time
    // walls.spot = current SPY/QQQ price from FreeFlow (most accurate)
    // priceCache has ES/NQ from Yahoo Finance
    ...
    if (ffSpot && ffSpot > 0) {
      ...
      if (futuresPrice) {
        conversionRatio = futuresPrice / ffSpot;
        console.log(`GEX ${symbol} ratio: ${conversionRatio.toFixed(4)} (futures=${futuresPrice} spot=${ffSpot})`);
      }
    }

    const convert = (price) => {
      if (!price) return null;
      if (conversionRatio) return +(price * conversionRatio).toFixed(2);
      // Static fallback
      return symbol === 'SPY' ? +(price * 10.10).toFixed(2) : +(price * 41.267).toFixed(2);
    };

    const result = {
      call_wall:        convert(walls.call_wall?.strike) ?? null,
      put_wall:         convert(walls.put_wall?.strike)  ?? null,
      gamma_flip:       convert(typeof walls.gamma_flip === 'number' ? walls.gamma_flip : walls.gamma_flip?.strike) ?? null,
      net_gex:          snap.total_gex  ?? null,
      net_dex:          snap.total_dex  ?? null,
      net_vanna:        snap.total_dag  ?? null,
      net_ag:           snap.total_ag   ?? null,
      underlying_price: convert(ffSpot) ?? null,
      spy_spot:         ffSpot ?? null,
      net_gex_label:    null,
      expiration:       exp
    };
...
// Conversion is done at fetchGEX time — no conversion needed here
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  const formatGex = (cache, symbol) => {
    ...
    return {
      ...
      gamma_flip:        d.gamma_flip ?? null,
      call_wall:         d.call_wall ?? null,
      put_wall:          d.put_wall ?? null,
      underlying_price:  d.underlying_price ?? null,
