...
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  const formatGex = (cache, symbol) => {
    ...
    // FreeFlow already returns levels in ES/NQ equivalent prices — no ratio conversion needed
    const round2 = (v) => v !== null && v !== undefined ? +parseFloat(v).toFixed(2) : null;
    ...
    return {
      ...
      gamma_flip:        round2(d.gamma_flip_raw),
      call_wall:         round2(d.call_wall_raw),
      put_wall:          round2(d.put_wall_raw),
      underlying_price:  d.ff_spot ? round2(d.ff_spot) : null,
