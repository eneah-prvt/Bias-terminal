...
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  const formatGex = (cache, symbol) => {
    ...
    return {
      symbol,
      net_gex:           d.net_gex ?? d.exposures?.net_gex ?? null,
      net_gex_label:     d.regime ?? d.net_gex_label ?? null,
      gamma_flip:        d.gamma_flip ?? d.levels?.gamma_flip ?? null,
      call_wall:         d.call_wall ?? d.levels?.call_wall ?? null,
      put_wall:          d.put_wall ?? d.levels?.put_wall ?? null,
      underlying_price:  d.underlying_price ?? d.price?.last ?? null,
      updatedAt:         cache.updatedAt
    };
  };
  res.json({
    SPY: formatGex(gexCache.SPY, 'SPY'),
    QQQ: formatGex(gexCache.QQQ, 'QQQ'),
    timestamp: new Date().toISOString()
  });
});
