...
async function fetchFredSeries(seriesId, units) {
  ...
  try {
    ...
    const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${seriesId}&api_key=${FRED_KEY}&file_type=json&sort_order=desc&limit=3${unitsParam}`;
...
async function fetchMacro() {
  try {
    const [vix, t10raw, t2raw, dxy, hy, indpro, cpi] = await Promise.all([
      ...
      fetchFredSeries('INDPRO', 'pc1'),   // Industrial Production YoY % (Growth)
      fetchFredSeries('CPIAUCSL', 'pc1'), // CPI YoY % (Inflation rate, not index level)
    ]);
    ...
    if (indpro !== null && cpi !== null) {
      // INDPRO YoY %: > 0 = expansion, < 0 = contraction
      const growthExpanding = indpro > 0;
      // CPI YoY %: > 3.0% = elevated inflation (Fed target 2%, buffer to 3%)
...
async function fetchCOT() {
  ...
  const url = `https://publicreporting.cftc.gov/resource/gpe5-46if.json?$where=report_date_as_yyyy_mm_dd>'2020-01-01' AND (cftc_contract_market_code='13874%2B' OR cftc_contract_market_code='209742')&$order=report_date_as_yyyy_mm_dd DESC&$limit=120`;
  try {
    ...
    const processInstrument = (code, name) => {
      ...
      return {
        ...
        leveragedFunds: { net: net('lev_money_positions_long','lev_money_positions_short'),   index: cotIndex('lev_money_positions_long','lev_money_positions_short') },
        assetManagers:  { net: net('asset_mgr_positions_long','asset_mgr_positions_short'),   index: cotIndex('asset_mgr_positions_long','asset_mgr_positions_short') },
...
const expListCache = { SPY: [], QQQ: [] };
const regimeEvents = []; // rolling log of GEX flips + flip crosses
...
async function fetchGEX(symbol) {
  ...
  try {
    let exp = await getExpiration(symbol);
    if (!exp) { console.warn(`GEX ${symbol}: no expiration found`); return null; }

    let { walls, snap } = await fetchGEXForExp(symbol, exp);

    // 0DTE data may be empty before market open — fallback to next expiration
    const wallsEmpty = !walls.call_wall && !walls.put_wall && walls.gamma_flip == null;
    if (wallsEmpty && expListCache[symbol] && expListCache[symbol].length > 1) {
      const today = new Date().toISOString().split('T')[0];
      const nextExp = expListCache[symbol].find(e => e > today);
      if (nextExp && nextExp !== exp) {
        console.log(`GEX ${symbol}: 0DTE (${exp}) empty pre-market — falling back to ${nextExp}`);
        exp = nextExp;
        ({ walls, snap } = await fetchGEXForExp(symbol, exp));
      }
    }
    ...
    // ── Aggregate per-strike rows: Net Charm, ATM IV, IV Skew, GEX Profile ──
    let netCharm = null, atmIV = null, ivSkew = null, gexProfile = null;
    if (snap.rows && snap.rows.length && ffSpot) {
      // Net Charm: sum of charm exposure across all strikes (dealer dDelta/dTime)
      netCharm = snap.rows.reduce((s, r) => s + (r.charmex || 0), 0);

      // ATM IV: average IV of the call+put closest to spot
      const byDist = [...snap.rows].sort((a, b) => Math.abs(a.strike - ffSpot) - Math.abs(b.strike - ffSpot));
      const atmRows = byDist.slice(0, 4).filter(r => r.iv_pct > 0);
      if (atmRows.length) atmIV = +(atmRows.reduce((s, r) => s + r.iv_pct, 0) / atmRows.length).toFixed(2);

      // IV Skew: OTM put IV (~2% below spot) minus OTM call IV (~2% above spot)
      const putTarget = ffSpot * 0.98, callTarget = ffSpot * 1.02;
      const otmPuts  = snap.rows.filter(r => r.right === 'P' && r.iv_pct > 0)
        .sort((a, b) => Math.abs(a.strike - putTarget) - Math.abs(b.strike - putTarget));
      const otmCalls = snap.rows.filter(r => r.right === 'C' && r.iv_pct > 0)
        .sort((a, b) => Math.abs(a.strike - callTarget) - Math.abs(b.strike - callTarget));
      if (otmPuts.length && otmCalls.length) {
        ivSkew = +(otmPuts[0].iv_pct - otmCalls[0].iv_pct).toFixed(2);
      }

      // GEX Profile: net GEX by strike, ±3% around spot, max 24 strikes
      const byStrike = {};
      for (const r of snap.rows) {
        if (Math.abs(r.strike - ffSpot) / ffSpot <= 0.03) {
          byStrike[r.strike] = (byStrike[r.strike] || 0) + (r.gex || 0);
        }
      }
      gexProfile = Object.entries(byStrike)
        .map(([k, v]) => ({ strike: +k, gex: v }))
        .sort((a, b) => a.strike - b.strike);
      if (gexProfile.length > 24) {
        // keep the 24 strikes closest to spot
        gexProfile = gexProfile
          .sort((a, b) => Math.abs(a.strike - ffSpot) - Math.abs(b.strike - ffSpot))
          .slice(0, 24)
          .sort((a, b) => a.strike - b.strike);
      }
    }
    ...
    const result = {
      ...
      call_wall_gex:    walls.call_wall?.gex ?? null,
      put_wall_gex:     walls.put_wall?.gex ?? null,
      gamma_flip_raw:   typeof walls.gamma_flip === 'number' ? walls.gamma_flip : walls.gamma_flip?.strike ?? null,
      call_oi_wall_raw: walls.call_oi_wall?.strike ?? null,
      put_oi_wall_raw:  walls.put_oi_wall?.strike ?? null,
      call_oi:          walls.call_oi_wall?.oi ?? null,
      put_oi:           walls.put_oi_wall?.oi ?? null,
    ...
    // ── Regime event detection: GEX sign flip + price/flip cross ──
    const prevData = gexCache[symbol]?.data;
    if (prevData) {
      const ts = new Date().toISOString();
      if (prevData.net_gex != null && result.net_gex != null && Math.sign(prevData.net_gex) !== Math.sign(result.net_gex)) {
        regimeEvents.unshift({ ts, symbol, type: 'GEX_FLIP', msg: `Net GEX flipped ${result.net_gex > 0 ? 'POSITIVE' : 'NEGATIVE'} (${(result.net_gex/1e9).toFixed(2)}B)` });
      }
      if (prevData.gamma_flip_raw && result.gamma_flip_raw && prevData.ff_spot && result.ff_spot) {
        const wasAbove = prevData.ff_spot > prevData.gamma_flip_raw;
        const isAbove  = result.ff_spot > result.gamma_flip_raw;
        if (wasAbove !== isAbove) {
          regimeEvents.unshift({ ts, symbol, type: 'FLIP_CROSS', msg: `Price crossed ${isAbove ? 'ABOVE' : 'BELOW'} gamma flip` });
        }
      }
      if (regimeEvents.length > 30) regimeEvents.length = 30;
    }
...
// Update GEX every minute (weekdays only)
...
console.log('GEX scheduler active: every minute on weekdays');
...
async function fetchVWAP() {
  ...
  try {
    ...
    // Use live price-based ratio (futures price / underlying spot) for accuracy
    // This matches the ratio used for GEX level conversion, avoiding VWAP drift
    const esSpot = spyData?.candles?.length ? spyData.candles[spyData.candles.length - 1].close : null;
    const nqSpot = qqqData?.candles?.length ? qqqData.candles[qqqData.candles.length - 1].close : null;
    const esPrice = priceCache.ES?.price;
    const nqPrice = priceCache.NQ?.price;
    const esRatio = (esPrice && esSpot) ? esPrice / esSpot : (dailyRatio.SPY || 10.03);
    const nqRatio = (nqPrice && nqSpot) ? nqPrice / nqSpot : (dailyRatio.QQQ || 41.30);
