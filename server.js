...
async function getExpiration(symbol) {
  try {
    ...
    // Skip 0DTE (today) — use next available expiration for more stable levels
    // 0DTE levels are too volatile and change every minute
    const future = exps.filter(e => e > today);
    return future[0] || exps[0] || null;
  } catch(e) { return null; }
}
...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    const agParam = '';  // ag=on doesn't convert strikes, removed
    ...
    const result = {
      ...
      spy_spot:         walls.spot ?? snap.spot ?? null,  // Raw SPY/QQQ spot for ratio calculation
      net_gex_label:    null,
      expiration:       exp
    };
...
function spyToES(price) {
  if (!price) return null;
  const esPrice = priceCache.ES?.price;
  const spySpot = gexCache.SPY?.data?.spy_spot;
  if (esPrice && spySpot && spySpot > 0) {
    const ratio = esPrice / spySpot;
    return +(price * ratio).toFixed(2);
  }
  return +(price * 10.10).toFixed(2); // fallback
}

// Dynamic QQQ→NQ conversion using live prices  
function qqqToNQ(price) {
  if (!price) return null;
  const nqPrice = priceCache.NQ?.price;
  const qqqSpot = gexCache.QQQ?.data?.spy_spot;
...
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  const formatGex = (cache, symbol, convertFn) => {
    ...
    return {
      ...
      call_wall:         convertFn(d.call_wall),
      put_wall:          convertFn(d.put_wall),
