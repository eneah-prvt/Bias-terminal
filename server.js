...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    const result = {
      ...
      net_vanna:        snap.total_dag  ?? null,  // total_dag = delta-adjusted gamma ≈ vanna proxy
...
// SPY × 10.0338 ≈ ES, QQQ × 41.267 ≈ NQ (based on ratio calculator)
function spyToES(price) { return price ? +(price * 10.0338).toFixed(2) : null; }
function qqqToNQ(price) { return price ? +(price * 41.267).toFixed(2) : null; }
