...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    // snap returns array of contracts + totals
    const totals = Array.isArray(snap) ? snap.find(s => s.total_gex !== undefined) : snap;

    const result = {
      call_wall:        walls.call_wall?.strike ?? null,
      put_wall:         walls.put_wall?.strike  ?? null,
      gamma_flip:       walls.gamma_flip?.strike ?? null,
      call_wall_exp:    walls.call_wall?.exposure ?? null,
      put_wall_exp:     walls.put_wall?.exposure  ?? null,
      net_gex:          totals?.total_gex ?? null,
      net_dex:          totals?.total_dex ?? null,
      net_vanna:        totals?.total_vanna ?? null,
      underlying_price: totals?.spot ?? snap?.spot ?? null,
      net_gex_label:    null, // calculated below
      expiration:       exp
    };
