...
async function fetchCOT() {
  // FMP COT API — replaces CFTC Socrata (was returning empty rows)
  const FMP_KEY = process.env.FMP_API_KEY;
  if (!FMP_KEY) { console.warn('FMP_API_KEY not set'); return; }

  const fetchFMP = async (symbol) => {
    const url = `https://financialmodelingprep.com/api/v4/commitment_of_traders_report/${symbol}?apikey=${FMP_KEY}`;
    const r = await fetch(url, { headers: { 'Accept': 'application/json' } });
    if (!r.ok) throw new Error(`FMP COT ${symbol} HTTP ${r.status}`);
    return await r.json();
  };

  try {
    const [esData, nqData] = await Promise.all([fetchFMP('ES'), fetchFMP('NQ')]);

    const processInstrument = (data, name) => {
      if (!data || !data.length) return null;
      const history = data.slice(0, 52);
      const latest = history[0];
      console.log('COT FMP fields:', Object.keys(latest).join(', ').slice(0, 300));
      ...
      return {
        ...
        leveragedFunds: { net: net('leveragedFundsLong','leveragedFundsShort'),             index: cotIndex('leveragedFundsLong','leveragedFundsShort') },
        assetManagers:  { net: net('assetManagerLong','assetManagerShort'),                  index: cotIndex('assetManagerLong','assetManagerShort') },
        smallSpec:      { net: net('nonreportablePositionsLong','nonreportablePositionsShort'), index: cotIndex('nonreportablePositionsLong','nonreportablePositionsShort') }
      };
    };

    cotCache.data = {
      ES: processInstrument(esData, 'S&P 500 E-mini'),
      NQ: processInstrument(nqData, 'NASDAQ 100 E-mini'),
      timestamp: new Date().toISOString()
    };
    cotCache.updatedAt = new Date().toISOString();
    console.log('COT data updated:', cotCache.updatedAt);
    return; // skip old CFTC code below
  } catch (e) {
    console.warn('COT FMP error:', e.message);
  }
}

async function fetchCOT_OLD_UNUSED() {
