...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    // ── Aggregate per-strike rows: Net Charm, ATM IV, IV Skew ──
    let netCharm = null, atmIV = null, ivSkew = null;
...
async function fetchVWAP() {
  if (!FF_KEY) return;
  try {
    // Fetch 1min bars for SPY and QQQ from FreeFlow
    ...
    vwapCache.ES = { bands: calcVWAP(spyData, esRatio), updatedAt: new Date().toISOString() };
    vwapCache.NQ = { bands: calcVWAP(qqqData, nqRatio), updatedAt: new Date().toISOString() };
