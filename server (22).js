...
async function fetchAllPrices() {
  ...
  // Update ratios immediately after price fetch
...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    const [wallsRes, snapRes] = await Promise.all([
      // Walls without exp = aggregated across all expirations (matches FreeFlow website)
      // Snapshot with today's exp = current GEX/DEX totals
      fetch(`https://www.free-flow.site/public/walls?symbol=${symbol}`, { headers: { 'X-API-Key': FF_KEY } }),
