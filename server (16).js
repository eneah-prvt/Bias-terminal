...
const dailyRatio = { SPY: 10.085, QQQ: 41.30, date: null };
...
function updateDailyRatio(symbol, ffSpot) {
  ...
  // Update ratio whenever Yahoo Finance has a fresh price (during market hours)
  // Outside market hours ratio stays from last update — very stable day to day
  if (utcH < 13 || utcH >= 22) return;
  const newRatio = futuresPrice / ffSpot;
  if (newRatio > 9 && newRatio < 12 && symbol === 'SPY') {
    dailyRatio.SPY = newRatio;
    dailyRatio.date = today;
  }
  if (newRatio > 38 && newRatio < 45 && symbol === 'QQQ') {
    dailyRatio.QQQ = newRatio;
  }
}
...
async function getExpiration(symbol) {
  try {
    ...
    const exp = future[0] || exps[0] || null;
    if (exp) lastExp[symbol] = exp; // cache it
...
async function fetchGEX(symbol) {
  ...
  try {
    ...
    // Update price cache with FreeFlow spot * ratio (always current)
    if (result.ff_spot && result.stored_ratio) {
      const convertedPrice = +(result.ff_spot * result.stored_ratio).toFixed(2);
      const cacheKey = symbol === 'SPY' ? 'ES' : 'NQ';
      if (!priceCache[cacheKey]) priceCache[cacheKey] = {};
      priceCache[cacheKey].price = convertedPrice;
      priceCache[cacheKey].updatedAt = new Date().toISOString();
      console.log('Price ' + cacheKey + ' updated from FreeFlow: ' + convertedPrice);
    }
    console.log('GEX ' + symbol + ' updated: flip=' + result.gamma_flip + ' call=' + result.call_wall + ' put=' + result.put_wall + ' spot=' + result.underlying_price);
    return result;
  } catch(e) {
...
app.get('/api/gex', requireAuth, requireSubscription, (req, res) => {
  const formatGex = (cache, symbol) => {
    ...
    // Use FreeFlow spot for ratio — this is fetched every minute so always current
    const ffSpot = d.ff_spot;
    const futuresPrice = symbol === 'SPY' ? priceCache.ES?.price : priceCache.NQ?.price;
    let ratio = symbol === 'SPY' ? 10.10 : 41.267; // static fallback
    if (ffSpot && ffSpot > 0 && futuresPrice) {
      ratio = futuresPrice / ffSpot;
    }
...
app.get('/api/gex/raw', async (req, res) => {
  res.json({
    current_ratios: dailyRatio,
    spy_cache: gexCache.SPY?.data ? {
      ff_spot: gexCache.SPY.data.ff_spot,
      stored_ratio: gexCache.SPY.data.stored_ratio,
      call_raw: gexCache.SPY.data.call_wall_raw,
      put_raw: gexCache.SPY.data.put_wall_raw,
      flip_raw: gexCache.SPY.data.gamma_flip_raw,
      updatedAt: gexCache.SPY.updatedAt
    } : null,
    qqq_cache: gexCache.QQQ?.data ? {
      ff_spot: gexCache.QQQ.data.ff_spot,
      stored_ratio: gexCache.QQQ.data.stored_ratio,
      updatedAt: gexCache.QQQ.updatedAt
    } : null,
    price_cache: { ES: priceCache.ES?.price, NQ: priceCache.NQ?.price }
  });
});
